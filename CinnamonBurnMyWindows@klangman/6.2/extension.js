...
'use strict';
const {Workspace} = require('/usr/share/cinnamon/js/ui/workspace.js');
      ...
      Main.wm._shouldAnimate = function(actor, types) {
         ...
         if (isNormalWindow || isDialogWindow) {
            ...
            if (forClosing || forOpening) {
               ...
               const chosenEffect = {effect: extensionThis._ALL_EFFECTS[effectIdx], profile: extensionThis._settings}; //extensionThis._chooseEffect(actor, forOpening);

               if (chosenEffect) {
                  //log( `chosenEffect: ${chosenEffect.effect.constructor.getNick()} for ${forOpening}, ${forClosing}.. easing duration: ${actor.get_easing_duration()}` );
...
*/
  ...
  _chooseEffect(actor, forOpening) {
    ...
    if (!isNormalWindow && !isDialogWindow) {
      log( "Window is not normal or a dialog!" );
      return null;
    }
    ...
    const previewNick = this._settings.getValue('preview-effect');

    if (previewNick != '') {
      const activeProfile = this._settings.getValue('active-profile');
      effect =
        this._ALL_EFFECTS.find(effect => effect.constructor.getNick() == previewNick);
      profile = null; //this._profiles.find(p => p.path == activeProfile);

      // Only preview the effect until the preview window is closed.
      if (!profile || !forOpening) {
        this._settings.setValue('preview-effect', '');
      }
    }
    ...
    // effect.
      ...
      profile = this._profiles.find(p => {
        const profileApp           = p.settings.getValue('profile-app');
        ...
        if (matches && profileColorScheme != 0) {
          const colorScheme = this._shellSettings.getValue('color-scheme');
  ...
  _setupEffect(actor, forOpening, effect, profile) {
    ...
    const duration = testMode ?
      8000 :
      profile./*settings.*/getValue(effect.constructor.getNick() + '-animation-time');
